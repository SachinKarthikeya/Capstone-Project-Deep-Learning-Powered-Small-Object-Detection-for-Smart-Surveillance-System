...
# CONFIG
# =========================
ANN_PATH = "/Users/sachinkarthikeya/Downloads/Capstone Project/ssd-knives-detection-csv/SOD-SSS-5-CSV/valid/_annotations.csv"
...
model.load_state_dict(torch.load("mobilenet_ssd_model.pth", map_location=device))
model.eval()
...
# PLOTS (YOLO STYLE)
# =========================
def compute_pr_curve(predictions, gts):
    return compute_ap_data(predictions, gts)


def compute_ap_data(predictions, gts):
    predictions = sorted(predictions, key=lambda x: x["score"], reverse=True)
...
recalls, precisions = compute_ap_data(all_predictions, all_gts)
...
plt.tight_layout()
plt.savefig("results.png")  # 🔥 FINAL OUTPUT
plt.show()
